    ...
    <v-row>
      <v-col cols="12" md="6">
        <v-card id="card-ability" class="elevation-10">
          ...
          <v-card-subtitle>自分のデッキに設定している特攻数を設定してください (合計{{this.maxTotalEventAbilityCard}}枚まで)。</v-card-subtitle>
          <v-card-text>
            ...
            <v-row>
              <v-col offset="6" cols="4" class="text-right">
                合計
              </v-col>
              <v-col cols="2">
                <span v-bind:class="{ 'error--text': hasTotalEventAbilityCardWarning }">{{ totalEventAbilityCard }}</span>
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="6" class="mt-0 pt-0">
                <v-select label="サポ" :items="eventSupporterSelect" v-model="inputs.eventAbilitySupporter.selectedValue"></v-select>
              </v-col>
              <v-col offset="1" cols="5" class="mt-0 pt-0">
                <v-switch label="ギルド外" v-model="inputs.eventAbilitySupporter.externalSupporter"></v-switch>
              </v-col>
            </v-row>
            <v-row>
              <v-col>
                ...
                <div v-else>
                  <p class="mb-0">
                    ...
                    <span v-bind:class="{ 'error--text': hasTotalMagicStoneWarning }">{{ totalMagicStone }}</span>
      ...
      <v-col cols="12" md="6">
        <v-card class="elevation-10">
          ...
          <v-card-subtitle>通常周回とチャンスボスのドロップ数を計算します。</v-card-subtitle>
          <v-card-text>
            <v-row>
              <v-col>
                <v-data-table :headers="dropCountHeader" :items="dropCountTable" :items-per-page="5"
                  hide-default-footer disable-sort></v-data-table>
              </v-col>
            </v-row>
          </v-card-text>
          <v-card-subtitle>※通常周回は特攻分のドロップ数のみ表示しています。実際のドロップ数は特攻分＋ステージドロップ(ランダム)です。</v-card-subtitle>
        </v-card>
      </v-col>
    </v-row>
    <v-row>
      <v-col cols="12" md="6">
        <v-card class="elevation-10">
          ...
          <v-card-subtitle>1やる気あたりのドロップ数を計算します。</v-card-subtitle>
          <v-card-text>
            <v-row>
              <v-col>
                <v-select label="周回するステージ" :items="stageLevel"
                  v-model="inputs.normalStageLevel" dense></v-select>
              </v-col>
            </v-row>
            <v-row>
            ...
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="周回時のドロップ数(特攻分)" class="mt-0 pt-0" type="number"
                  v-model.number="normalStageCardDrop" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="周回時の合計ドロップ数" class="mt-0 pt-0" type="number"
                  v-model.number="totalNormalStageDrop" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="チャンスボスが出現するまでの周回数" class="mt-0 pt-0" type="number"
                   v-model.number="inputs.normalStageLoop" dense outlined>
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-select label="チャンスボス難易度" :items="stageLevel"
                  v-model="inputs.chanceBossLevel" dense></v-select>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="チャンスボスのドロップ数" class="mt-0 pt-0" type="number"
                  v-model.number="totalChanceBossDrop" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="1セットのドロップ数" class="mt-0 pt-0" type="number"
                  v-model.number="totalDropOfCycle" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="1セットの消費やる気合計" class="mt-0 pt-0" type="number"
                  v-model.number="totalStaminaOfCycle" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="1やる気あたりのドロップ数" class="mt-0 pt-0" type="number"
                  v-model.number="dropPerStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n6">
                <p>※通常ステージを周回してチャンスボスを討伐するまでを1セットとします。</p>
              </v-col>
            </v-row>
          </v-card-text>
        </v-card>
      ...
      <v-col cols="12" md="6">
        <v-card id="magic-stone" class="elevation-10">
          ...
          <v-card-subtitle>やる気効率から目標までに必要な魔導石の数を計算します。</v-card-subtitle>
          <v-card-text>
            <v-row>
              <v-col>
                <v-text-field label="目標収集数" class="mt-0 pt-0" type="number"
                  v-model.number="inputs.goal" dense outlined>
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="目標までに必要な総やる気" class="mt-0 pt-0" type="number"
                  v-model.number="totalStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
            ...
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="やる気草分のやる気" class="mt-0 pt-0" type="number"
                  v-model.number="totalItemStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="やる気上限" class="mt-0 pt-0" type="number"
                  v-model.number="inputs.maxStamina" dense outlined>
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="イベント中の想定ランクアップ回数" class="mt-0 pt-0" type="number"
                  v-model.number="inputs.numOfRankUp" dense outlined>
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="ランクアップ回復分のやる気" class="mt-0 pt-0" type="number"
                  v-model.number="totalRankUpStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="イベント日数" class="mt-0 pt-0" type="number"
                  v-model.number="inputs.totalDaysOfEvent" dense outlined>
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="イベント期間の自然回復分のやる気" class="mt-0 pt-0" type="number"
                  v-model.number="totalNaturalStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="イベント開始時のやる気" class="mt-0 pt-0" type="number"
                  v-model.number="inputs.startStamina" dense outlined>
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="無料分のやる気(草+RU+自然回復+開始時)" class="mt-0 pt-0" type="number"
                  v-model.number="totalFreeStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="魔導石未使用時の収集数" class="mt-0 pt-0" type="number"
                  v-model.number="totalDropOfFreeStamina" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="不足分のやる気" class="mt-0 pt-0" type="number"
                  v-model.number="totalStaminaGap" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="mt-n2 pt-0">
                <v-text-field label="必要な魔導石" class="mt-0 pt-0" type="number"
                  v-model.number="totalMagicStone" dense outlined readonly background-color="grey lighten-4">
                </v-text-field>
              </v-col>
            </v-row>
          </v-card-text>
  ...
  export default {
    ...
    computed: {
      ...
      totalDropOfCycle: function () {
        return this.totalNormalStageDrop * this.inputs.normalStageLoop + this.totalChanceBossDrop
      },
      totalStaminaOfCycle: function () {
        return this.normalStageStamina * this.inputs.normalStageLoop + this.chanceBossStamina
      },
      ...
      totalHoursOfEvent: function () {
        if (this.inputs.totalDaysOfEvent <= 0)
          return 0
    ...
    methods: {
      calcNormalStageCardDropDict: function () {
        return this.calcDropDict("通常周回(特攻分のみ)", this.calcNormalStageCardDrop)
      },
      calcChanceBossCardDropDict: function () {
        return this.calcDropDict("チャンスボス(特攻分のみ)", this.calcChanceBossCardDrop)
      },
      calcChanceBossTotalDropDict: function () {
        return this.calcDropDict("チャンスボス(合計)", this.calcChanceBossTotalDrop)
      },
      ...
      calcChanceBossCardDrop: function (level, cardNums, supporter) {
        let normal_drop = this.calcNormalStageCardDrop(level, cardNums, supporter)
